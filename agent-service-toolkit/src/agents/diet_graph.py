...
# --- 노드 함수들 ---
from .diet_loader import hybrid_retriever
...
compiled_diet_graph = flow.compile(checkpointer=memory)

def diet_graph(inputs, config):
    final_output = None
    for output in compiled_diet_graph.stream(inputs, config, stream_mode="updates"):
        final_output = output
    return final_output
