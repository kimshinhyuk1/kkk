...
# --- 노드 함수들 ---
from .general_loader import hybrid_retriever
...
compiled_general_graph = flow.compile(checkpointer=memory)

def general_graph(inputs, config):
    final_output = None
    for output in compiled_general_graph.stream(inputs, config, stream_mode="updates"):
        final_output = output
    return final_output
